			...
			{!linkValidator ? (
				...
				>
					...
					<Form.Item
						label="טלפון"
						name="phone"
						rules={[{ required: true, message: "הכנס מספר טלפון תקין" }]}
					>
						<Input
							id="phoneNumber"
					...
					>
						<Input.Password
							onChange={e => {
								setHrData({ ...hrData, password1: e.target.value });
							}}
						/>
					</Form.Item>
					...
					<Form.Item label="תמונת פרופיל" name="profilePic">
						<FileBase
							...
							multiple={false}
							onDone={({ base64 }: any) => {
								setHrData({ ...hrData, profilePicture: base64 });
							}}
						/>
					</Form.Item>
					...
					{hrData.profilePicture && (
						<Image width={200} src={hrData.profilePicture} />
					)}
				</Form>
			)}
		</>
